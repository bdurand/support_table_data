...
- `:compact` — the same `@!method` and `@!return` tags per generated method as `:full`, but without the prose descriptions. YARD resolves it to exactly the same methods and return types as `:full`. Useful when a model has many named instances and the verbose comment block is too long to be useful inline.
